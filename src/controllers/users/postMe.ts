...
const postMe: RequestHandler = async (req, res) => {
  ...
  if (
    ...
    const targetByUser: any = await userServ.getMe(datas.username)
    ...
    let problem: any = ""
    if (
      targetByEmail.length > 0 ||
      targetByPhone.length > 0 ||
      targetByUser.length > 0
    ) {
      problem += targetByEmail.length > 0 ? "Email, " : ""

      problem += targetByPhone.length > 0 ? "Phone, " : ""
      problem += targetByUser.length > 0 ? "UserName, " : ""
